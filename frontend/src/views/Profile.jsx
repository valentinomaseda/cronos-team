  ...
  return (
    <div className="page-shell space-y-6">
      <div className="flex items-center space-x-3 animate-slide-in-left">
        <UserCircle className="text-brandBlue" size={28} strokeWidth={2.5} />
      ...
      <div className="surface-brand p-6 space-y-6 animate-slide-in-up delay-100">
        <div className="flex flex-col items-center animate-scale-in delay-200">
          <div className="w-32 h-32 bg-brandBlue rounded-full flex items-center justify-center mb-4 overflow-hidden">
            {user?.photo ? (
              <img src={user.photo} alt={user.nombre} className="w-full h-full object-cover" />
            ) : (
              <UserCircle size={80} className="text-text" strokeWidth={2} />
            )}
          </div>
          <h3 className="text-2xl font-bold text-text">{user?.nombre || 'Coach Admin'}</h3>
          <p className="text-brandBlue mt-1">
        ...
        <div className="space-y-4 pt-6 border-t border-border">
          <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-300">
            <Mail className="text-brandBlue" size={24} />
            <div>
              <p className="text-xs text-brandBlue">Email</p>
              <p className="font-semibold text-text">{user?.mail || 'coach@adrenalinaxtrema.com'}</p>
            </div>
          </div>

          <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-400">
            <Phone className="text-brandBlue" size={24} />
            <div>
              <p className="text-xs text-brandBlue">Teléfono</p>
              <p className="font-semibold text-text">{user?.tel || 'No especificado'}</p>
            </div>
          </div>
          ...
          {user?.rol === 'alumno' ? (
            <>
              {(user?.domicilio || user?.direccion) && (
                <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
                  <MapPin className="text-brandBlue" size={24} />
                  <div>
                    <p className="text-xs text-brandBlue">Dirección</p>
                    <p className="font-semibold text-text">{user.domicilio || user.direccion}</p>
                  </div>
                </div>
              )}
              ...
              {user?.fechaNacimiento && (
                <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
                  <Cake className="text-brandBlue" size={24} />
                  <div>
                    <p className="text-xs text-brandBlue">Fecha de Nacimiento</p>
              ...
              {user?.genero && (
                <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
                  <User className="text-brandBlue" size={24} />
                  <div>
                    <p className="text-xs text-brandBlue">Género</p>
                    <p className="font-semibold text-text">{user.genero === 'masculino' ? 'Masculino' : 'Femenino'}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                {user?.peso && (
                  <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
                    <Weight className="text-brandBlue" size={24} />
                    <div>
                      <p className="text-xs text-brandBlue">Peso</p>
                      <p className="font-semibold text-text">{user.peso} kg</p>
                    </div>
                  </div>
                )}
                ...
                {user?.altura && (
                  <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
                    <Ruler className="text-brandBlue" size={24} />
                    <div>
                      <p className="text-xs text-brandBlue">Altura</p>
                      <p className="font-semibold text-text">{user.altura} cm</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              {user?.nivel && (
                <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
                  <Award className="text-brandBlue" size={24} />
                  <div>
                    <p className="text-xs text-brandBlue">Nivel</p>
                    <p className="font-semibold text-text">{user.nivel}</p>
                  </div>
                </div>
              )}
            </>
            ...
            <div className="flex items-center space-x-3 p-4 bg-bg rounded-lg animate-slide-in-right delay-500">
              <Award className="text-brandBlue" size={24} />
              <div>
                <p className="text-xs text-brandBlue">Alumnos Activos</p>
                <p className="font-semibold text-text">{students?.length || 0} atletas</p>
              </div>
            </div>
          )}
        </div>
      ...
      {showEditModal && (
        <div className="modal-shell animate-fade-in">
          <div className="modal-panel animate-scale-in">
            <div className="flex-shrink-0 bg-brandBlue text-white p-6 flex items-center justify-between rounded-t-xl z-10">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="border-t border-border pt-4 mt-4">
                <h4 className="text-brandBlue font-bold mb-3">Cambiar Contraseña</h4>
                <p className="text-xs text-text-muted mb-4">Solo completa estos campos si deseas cambiar tu contraseña</p>
                
                <div className="space-y-3">
                  {/* Contraseña actual */}
                  <div>
                    <label className="block text-brandBlue font-semibold mb-2">Contraseña Actual</label>
                    <div className="relative">
                      <input
                        type={showCurrentPassword ? "text" : "password"}
                        value={editFormData.currentPassword || ''}
                        onChange={(e) => setEditFormData({...editFormData, currentPassword: e.target.value})}
                        className="w-full px-4 py-3 pr-12 bg-bg text-text border-2 border-border rounded-lg focus:border-border focus:outline-none"
                        placeholder="Ingresa tu contraseña actual"
                      />
                      <button
                        type="button"
                        onClick={() => setShowCurrentPassword(!showCurrentPassword)}
                        className="absolute right-3 top-1/2 transform -translate-y-1/2 text-text-muted hover:text-text transition"
                      >
                  ...
                  {/* Nueva contraseña */}
                  <div>
                    <label className="block text-brandBlue font-semibold mb-2">Nueva Contraseña</label>
                    <div className="relative">
                      <input
                  ...
                  <div>
                    <label className="block text-brandBlue font-semibold mb-2">Confirmar Nueva Contraseña</label>
                    <div className="relative">
                      <input
              ...
              <div className="flex space-x-3 mt-6">
                <button
                  ...
                  onClick={() => setShowEditModal(false)}
                  disabled={updating}
                  className="flex-1 px-4 py-3 bg-bg text-text rounded-lg hover:bg-bg-surface active:scale-95 transition-all font-semibold disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={updating}
                  className="flex-1 px-4 py-3 bg-brandBlue text-white rounded-lg hover:bg-brandBlue hover:text-white active:scale-95 transition-all font-semibold disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {updating ? (
                    <>
